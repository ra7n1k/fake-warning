...
const getBrowserInfo = () => {
  ...
  const browser = Object.keys(browserRegex).find(name => browserRegex[name].test(userAgent)) || 'Unknown';
  ...
  const userOs = Object.keys(osRegex).find(name => osRegex[name].test(userAgent)) || 'Unknown';

  const browserVersion = parseFloat((userAgent.match(/(?:Opera|OPR|Edge|Edg|MSIE|Chrome|Safari|Firefox)[\s/](\d+(\.\d+)?)/i) || [])[1]);

  return { browser, browserVersion, userOs };
};
